...
const initialState = {
  ...
  listOfProduct: localStorage.getItem("cartItem")? JSON.parse(localStorage.getItem("cartItem")):[],
...
const isExist = state.listOfProduct.find((i)=>i.datas.id === item.datas.id);
if(isExist){
  isExist.quantity+=1
  listOfProduct: state.listOfProduct.map((i)=>i.datas.id===item.datas.id?item:i)
}
else{
